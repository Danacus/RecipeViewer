...
var Settings = (_class = function () {
  ...
  _createClass(Settings, [{
    ...
    value: function loadSettings() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (_fsJetpack2.default.exists(file) !== "file") {
          _this.settings = { profiles: [], selectedProfile: 0 };
          resolve(null);
        } else {
    ...
    value: function readFile(path) {
      ...
      return new Promise(function (resolve, reject) {
        _fsJetpack2.default.readAsync(path, 'json').then(function (settings) {
          _this2.settings = { profiles: settings.profiles.map(function (profile) {
              return { name: profile.name, path: profile.path, networks: new _Networks2.default().deserialize(profile.networks) };
            }), selectedProfile: settings.selectedProfile };
          _this2.settings.profiles.forEach(function (profile) {
            console.log(profile);
            profile.networks.list.forEach(function (network) {
              network.recipes = _App.stores.recipes;
            });
          });
          resolve(_this2);
        });
    ...
    key: 'selectProfile',
    value: function selectProfile(index) {
    ...
    value: function saveSettings() {
      var file = app.getPath('userData') + "/settings.json";
      _fsJetpack2.default.writeAsync(file, { profiles: this.settings.profiles.map(function (profile) {
          return { name: profile.name, path: profile.path, networks: profile.networks.serialize() };
        }), selectedProfile: this.settings.selectedProfile });
    }
  }, {
    key: 'list',
    get: function get() {

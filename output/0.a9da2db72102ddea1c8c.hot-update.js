...
var HomePage = (0, _mobxReact.observer)(_class = function (_Component) {
  ...
  function HomePage(props) {
    ...
    _this.onChange = function (activeKey) {
      _this.props.settings.getCurrentProfile().networks.selectedNetwork = parseInt(activeKey);
    ...
    _this.remove = function (targetKey) {
      ...
      var lastIndex = -1;
      _this.props.settings.getCurrentProfile().networks.list.forEach(function (network, i) {
        if (network.id === targetKey) {
          lastIndex = i - 1;
        }
      });
      var networks = _this.props.settings.getCurrentProfile().networks.list.filter(function (network) {
        return network.id !== targetKey;
      });
      if (lastIndex >= 0 && activeKey === targetKey) {
        activeKey = networks[lastIndex].id;
      }
      _this.props.settings.getCurrentProfile().networks.networks = _this.props.settings.getCurrentProfile().networks.list.filter(function (network) {
        return network.id !== targetKey;
      });
      _this.setState({ activeKey: activeKey });
    };
    ...
    _this.newTabIndex = 0;
    _this.state = {
      activeKey: _this.props.settings.getCurrentProfile().networks.selectedNetwork.toString(),
      addProfile: false
    };
